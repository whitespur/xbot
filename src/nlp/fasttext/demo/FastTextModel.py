...
import jieba
from nlp.fasttext.demo.DataPretreatment import load_label_name_map,load_stop_word_list
...
def fasttext_model_train():
    ...
    start_time = time.time()
    classifier = ff.train_supervised("fasttext.train", epoch=epoch, lr=0.5, wordNgrams=wordNgrams)
    print("ngram=%d,训练第%d轮，用时%s" % (wordNgrams, epoch, time.time() - start_time))
    classifier.save_model("Model/model_w" + str(wordNgrams) + "_e" + str(epoch))
...
def load_model_to_test():
    ...
    # 加载分类模型
    # for w in range(1, 2):
    for w in range(wordNgrams, wordNgrams+1):
        all_marco_precision = []
        all_marco_recall = []
        all_marco_f1 = []
        all_micro_precision = []
        ...
        # for i in range(5, 51):
            classifier = ff.load_model("Model/model_w" + str(wordNgrams) + "_e" + str(epoch))
            ...
            for label, name in label_to_name.items():
                ...
                for i in range(len(texts)):
                    # 预测属于该类，实际属于该类
                    if predict_labels[i][0] == label and correct_labels[i] == label:
                        true_positive += 1
                    # 预测属于该类，实际不属于该类
                    elif predict_labels[i][0] == label and correct_labels[i] != label:
                        false_positive += 1
                    # 预测不属于该类，实际属于该类
                    elif predict_labels[i][0] != label and correct_labels[i] == label:
                        false_negative += 1
